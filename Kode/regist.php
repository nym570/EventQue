            ...
            <form action="" class="registrasi_form" id="registarsi_form" method="post" enctype="multipart/form-data">
                <div class="row">
                    <div>
                        ...
                        <input type="text" name="fname" class="registrasi_input" placeholder="John" required="required">
                        <p id="fnamemalert"></p>
                    </div>
                    <div>
                        <label>Last Name</label>
                        <input type="text" name="lname" class="registrasi_input" placeholder="Doe" required="required">
                        <p id="lnamemalert"></p>
                    </div>
                </div>
                <div>
                    <label>Tanggal Lahir</label>
                    <input type="date" name="date" class="registrasi_input"  onfocus="this.max=new Date().toISOString().split('T')[0]">
                    <p id="tanggalalert"></p>
                </div>
                <div>
                    <label>Email</label>
                    <input type="email" name="email" class="registrasi_input" placeholder="dummy@gmail.com" required="required">
                </div>
                <div>
                    <label>Phone Number</label>
                    <input type="text" name="contact" class="registrasi_input" placeholder="62895621061193" required="required">
                    <p id="phonealert"></p>
                </div>
                <div>
                  <label>Username</label>
                  <input type="text" name="username" class="registrasi_input" placeholder="JohnDoe20" required="required">
                  <p id="useralert"></p>
              </div>
              <div>
              ...
              <div>
                ...
                <select name="akun" class="registrasi_input">
                  <option value="mahasiswa">Mahasiswa STIS</option>
                  <option value="eventMaker">Event Maker</option>
              </select>
              </div>
              ...
              <div>
                ...
                <input type="button" value="Batal" onclick= "location.href='login.php';" class="batal_button"></input>
                </form>
    ...
    if(isset($_POST['submit'])){
       ...
       $cek->execute(array($username));
        ...
        if($row){
          $isValid = false;
          echo "<script type ='text/JavaScript'>  
                     var tag = document.getElementById('useralert');
                     var text = document.createTextNode('Username telah ada!');
                     tag.appendChild(text);
                     document.getElementById('useralert').style.color = 'red';</script>"; 
        }
       if($isValid){
        move_uploaded_file($tempname, $folder);
        $hash_pass = md5($password);
        $stmt = $pdo->exec("insert into users(username,pass,fname,lname,tanggal_lahir,email,contact,jenis,foto) values('$username','$hash_pass','$fname','$lname','$tanggal','$email','$contact','$akun','$filename')");
